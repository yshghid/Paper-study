...
## Data normalization, t-test

Normalization was performed identically to the RMA algorithm in the R package, and differentially expressed genes between CRC group and the control group were identified using a t-test.
...
The nodes in the network were set to genes with a p-value < 0.05 and a q-value (FDR) < 0.1. The edges in the network were weighted according to the correlation coefficients of the expression levels of each node (gene).
...
def create_graph(filtered_genes, normalized_data, threshold=0.7):
    ...
    for i in range(len(genes)):
	    ...
	    # Edges are only added between two nodes if the absolute value of the correlation coefficient between them exceeds 0.7.
...
Three topological parameters (eccentricity, closeness, betweenness) were calculated to determine the node strength of each graph.
...
[2]: https://github.com/yshghid/Paper-study/blob/main/%231-RWR-Clique-Connectivity/readme-kor.md#outline
